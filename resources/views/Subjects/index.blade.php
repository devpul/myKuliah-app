...
@section('content')
    @php
        ...
        $averageCredits = $totalSubjects > 0 ? $totalCredits / $totalSubjects : 0;
    @endphp
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-900 mt-2">
                            {{ number_format($averageCredits, 1) }}</p>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-2">
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-semibold {{ $subject->color ?? 'bg-gray-400' }} bg-opacity-10 text-gray-900">
                                        {{ $subject->code ?? 'N/A' }}
                                    </span>
                                    <span class="text-xs text-gray-500">{{ $subject->credits }} SKS</span>
                                    <span class="text-xs text-gray-500">Semester {{ $subject->semester }}</span>
                                </div>
                                <h3 class="text-lg font-semibold text-gray-900">{{ $subject->name }}</h3>
                            </div>
                            <div
                                class="{{ $subject->color ?? 'bg-gray-400' }} h-10 w-10 rounded-lg bg-opacity-10 flex items-center justify-center">
                                <svg class="h-6 w-6 {{ $subject->color ?? 'bg-gray-400' }} text-opacity-100" fill="none"
                        ...
                        <div class="space-y-3 mb-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-900">{{ $subject->lecture_name ?? 'Not assigned' }}</p>
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-900">{{ $subject->room ?? 'N/A' }}</p>
                        ...
                        <div class="pt-4 border-t border-gray-200 flex items-center justify-end">
                            <div class="flex gap-2">
                                <a href="{{ route('subjects.edit', $subject->id) }}" class="p-1.5 text-gray-400 hover:text-primary-600 rounded hover:bg-gray-50">
                                ...
                                <form action="{{ route('subjects.destroy', $subject->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-1.5 text-gray-400 hover:text-red-600 rounded hover:bg-gray-50" onclick="return confirm('Are you sure you want to delete this subject?')">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
