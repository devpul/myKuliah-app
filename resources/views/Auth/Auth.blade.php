...
<body>
    ...
    <!-- Main Content -->
        ...
        <div class="relative z-10 bg-white/10 backdrop-blur-lg p-10 rounded-2xl shadow-2xl w-full max-w-lg border border-white/20">
            ...
            @if(session('failed'))
                <div class="mb-5">
                    <div class="flex items-center justify-between px-4 py-3 rounded-lg bg-red-500/20 backdrop-blur-md border border-red-400/30 text-red-200 shadow-lg animate-fade-in-down">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 mr-2 text-red-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12l2 2 4-4m5 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <span class="font-semibold">{{ session('failed') }}</span>
                        </div>

                        <button onclick="this.parentElement.parentElement.remove()"
                            class="text-red-200 hover:text-red-300 transition">
                            X
                        </button>
                    </div>
                </div>
            ...
            <div class="flex mb-6 bg-white/20 rounded-lg p-1">
                <button id="login-tab" class="flex-1 py-2 px-4 text-center text-white font-medium rounded-md transition-all duration-300 {{ (request()->is('login') || request()->is('/')) ? 'bg-white/30 shadow-md' : 'hover:bg-white/10' }}">Login</button>
            ...
            <!-- Login Form -->
            <div id="login-form" class="transition-opacity duration-500 {{ (request()->is('login') || request()->is('/')) ? 'opacity-100' : 'opacity-0 hidden' }}">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="relative">
                            <input type="email" id="email" name="email" value="{{ old('email') }}" required
                                class="@error('email') border-red-500 @enderror w-full px-4 py-3 pl-12 bg-white/20 border border-white/30 rounded-lg text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all duration-300">
